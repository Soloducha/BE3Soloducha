...
class deliveriesService {
  ...
  async paginated({ page = 1, limit = 10 }) {
    ...
    const result = await deliveriesRepositorie.paginated({
      page: currentPage,
      limit: currentLimit,
    });
    const totalDocuments = await deliveriesRepositorie.countDocuments();
    ...
    return {
      ...
      prevLink:
        currentPage > 1
          ? `/api/deliveries?page=${currentPage - 1}&limit=${currentLimit}`
          : null,
      nextLink:
